...
- 구조 분해 할당 **우변**에는 **이터러블 객체** 무엇이든 올 수 있다.<br/>
    구조 분해 할당 **좌변**에는 **어떤 변수**든 올 수 있다. 즉, 객체의 속성 또한 올 수 있다.<br/>
    ```jsx
    // 우변
    let [a, b, c] = "abc"; // ["a", "b", "c"]
...
- 구조 분해 할당 시 우변에 사용된 이터러블 객체는 **변형되지 않는다.**<br/>
    ```jsx
    // 이 연산 이후에도 arr은 본 형태 그대로를 유지한다.
    let [firstName, surname] = arr;
    ```

- Object.entries()와 함께 사용하면 **객체의 key, value를 다른 변수에 담아 순회할 수 있다.**<br/>
    ```jsx
    const obj = {
        name: "nam",
        age: 25,
